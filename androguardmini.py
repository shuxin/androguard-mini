...
#coding=utf-8
#
# filename: androguard/core/bytecodes/apk.py
#
...
class StringBlock(object):
    ...
    def __init__(self, buff):
        ...
        if self.stringCount > buff.size():
            raise Exception("StringBlock self.stringCount > buff.size()")
        if self.styleOffsetCount > buff.size():
            raise Exception("StringBlock self.stringCount > buff.size()")
        ...
        if self.stylesOffset != 0:
            ...
            if buff.get_idx()+size>buff.size():
                size = size_backup

        # FIXME
        if (size % 4) != 0:
            pass#logging.warning("ooo")

        self.m_charbuff = buff.read_b(buff.size()-buff.get_idx())
        self.m_charbuff2 = buff.read(size)
        ...
        if self.stylesOffset != 0:
            ...
            if (size % 4) != 0:
                drop = buff.read(size % 4)

    def skipNullPadding(self, buff):

        header = unpack('<i', buff.read(4))[0]
        while header != CHUNK_STRINGPOOL_TYPE:
            if header == CHUNK_NULL_TYPE:
                header = unpack('<i', buff.read(4))[0]
                continue
            elif header == 1:
                header = unpack('<i', buff.read(4))[0]
    ...
    def decode_bytes(self, data, encoding, str_len):
        ...
        if len(string) != str_len:
            pass#logging.warning("invalid decoded string length")
        return string
...
class AXMLParser(object):
    ...
    def __init__(self, raw_buff):
        ...
        if True:#axml_file == CHUNK_AXML_FILE:
            self.buff.read(4)
            ...
            pass#logging.warning("Not a valid xml file")
        ...
        while True:
            ...
            if chunkType == CHUNK_RESOURCEIDS:
                ...
                if chunkSize < 8 or chunkSize % 4 != 0:
                    pass#logging.warning("Invalid chunk size")
            ...
            if chunkType < CHUNK_XML_FIRST or chunkType > CHUNK_XML_LAST:
                pass#logging.warning("invalid chunk type")
    ...
    def getXMLNS(self):
        ...
        for i in self.m_uriprefix:
            if i not in self.visited_ns:
                ...
                if self.sb.getString(self.m_prefixuri[self.m_uriprefix[i]]) == NS_ANDROID_URI:
                    self.xmlns_android = self.sb.getString(self.m_uriprefix[i])
    ...
    def getAttributeOffset(self, index):
        ...
        if self.m_event != START_TAG:
            pass#logging.warning("Current event is not START_TAG.")

        offset = index * 5
        # FIXME
        if offset >= len(self.m_attributes):
            pass#logging.warning("Invalid attribute index")
    ...
    def getAttributeNameSystem(self, index):
        offset = self.getAttributeOffset(index)
        name = self.m_attributes[offset + ATTRIBUTE_IX_NAME]
        try:
            attr = self.m_resourceIDs[name]
            if attr in public.FRAMEWORK_CORE_PUBLIC_ATTR_IDS:
                #res = self.xmlns_android + ':'+ public.FRAMEWORK_CORE_PUBLIC_ATTR_IDS[attr]
                res = public.FRAMEWORK_CORE_PUBLIC_ATTR_IDS[attr]
                return res
        except Exception,e:
            pass
        return None
...
def format_value(_type, _data, lookup_string=lambda ix: "<string>"):
    if _type == TYPE_STRING:
        ...
        if _data >> 24 == 1:
            if public.FRAMEWORK_CORE_PUBLIC_STYLE_IDS.has_key(_data):
                return public.FRAMEWORK_CORE_PUBLIC_STYLE_IDS[_data]
            else:
                return "?android:%08X" % (_data,)
        else:
            return "?%08X" % (_data,)
    ...
    def __init__(self, raw_buff):
        ...
        while True and self.axml.is_valid():
            ...
            if _type == START_DOCUMENT:
                ...
                self.buff += u'<' + self.getPrefix("" if self.axml.getPrefix() == self.axml.xmlns_android else self.axml.getPrefix(
                )) + self.axml.getName() + u' '
                self.buff += self.axml.getXMLNS()

                check_attrname_dup = set()
                for i in range(0, self.axml.getAttributeCount()):
                    if self.axml.getAttributeValueType(i) == 0xff:
                        continue
                    attrnamesys = self.axml.getAttributeNameSystem(i)
                    if attrnamesys:
                        attrname = ((self.axml.xmlns_android + ':') if self.axml.xmlns_android else "") + attrnamesys
                    else:
                        attrname = "%s%s" % (self.getPrefix(self.axml.getAttributePrefix(i)),self.axml.getAttributeName(i))
                    j = 0
                    attrname_dup = attrname
                    while attrname_dup in check_attrname_dup:
                        j += 1
                        attrname_dup = attrname + "_" + str(j)
                    check_attrname_dup.add(attrname_dup)
                    self.buff += "%s%s=\"%s\" " % (
                        "",
                        attrname_dup,
                ...
                self.buff += "</%s%s>" % (
                    self.getPrefix("" if self.axml.getPrefix() == self.axml.xmlns_android else self.axml.getPrefix()), self.axml.getName())
...
class ARSCParser(object):
    ...
    def __init__(self, raw_buff):
        ...
        for i in range(0, self.packageCount):
            ...
            while not self.buff.end():
                #print self.buff.get_idx(), self.buff.size()
                if self.buff.size() - self.buff.get_idx() < 8:
                    break
                ...
                if header.type == RES_TABLE_TYPE_SPEC_TYPE:
                    ...
                    #print [a_res_type,a_res_type.config.size,a_res_type.config,a_res_type.config.get_config_name()]
                    entries_0_and_1 = self.buff.read_b(8)
                    if a_res_type.config.size >= 52 and entries_0_and_1[:4] == entries_0_and_1[-4:] == "\x00\x00\x00\x00":
                        self.buff.read(4)
                        #i do not know why, but it works!
                        #print [entries_0_and_1[:4],entries_0_and_1[-4:]]
                    else:
                        pass
                        #print [entries_0_and_1[:4],entries_0_and_1[-4:]]
                    entries = []
                    ...
                    pass#logging.warning("unknown type")
                    break
    ...
    def get_dict(self):
        if self.shuxin:
            return self.id2value
        self.shuxin = True
        for package_name in self.packages:
            self.id2value[package_name] = {}
            nb = 3
            while nb < len(self.packages[package_name]):
                ...
                if isinstance(header, ARSCHeader):
                    if header.type == RES_TABLE_TYPE_TYPE:
                        ...
                        #print "%s%s/%s.xml" % ("values",("-" + a_res_type.config.get_config_name()) if a_res_type.config.get_config_name() else "",a_res_type.get_type()),"\t\t\t", repr(a_res_type.config)
                        entries = self.packages[package_name][nb + 2]
                        nb_i = 0
                        for entry, res_id in entries:
                            if entry != -1:
                                ate = self.packages[package_name][nb + 3 + nb_i]
                                if not self.id2value[package_name].has_key(ate.mResId):
                                    self.id2value[package_name][ate.mResId] = (a_res_type.get_type(),{})
                                self.id2value[package_name][ate.mResId][1][a_res_type.config.get_config_name()] = ate.get_key_data() if a_res_type.get_type() in {"string","drawable","mipmap"} else ate.get_value()
                                nb_i += 1
    ...
    def get_resource_dimen(self, ate):
        try:
            ...
            logging.debug("Out of range dimension unit index for %s: %s" % (
                complexToFloat(ate.key.get_data()),
    ...
    def __init__(self, buff=None, **kwargs):
        if buff is not None:
            ...
            if self.exceedingSize > 0:
                pass#logging.info("Skipping padding bytes!")
                self.padding = buff.read(self.exceedingSize)
            #print "[",self,[self.get_config_name()],self.start,self.size,self.exceedingSize,self.what_the_fuck,[self.padding],"]"
            #if self.size >= 52:
            #    self.what_the_fuck = unpack('<I', buff.read(4))[0]
        else:
            self.start = 0
            self.size = 0
            self.imsi = \
...
class ARSCComplex(object):
    ...
    def __init__(self, buff, parent=None):
        ...
        if self.count*4 > buff.size():
            raise Exception("ARSCComplex self.count > buff.size()")
...
if __name__ == "__main__":
    buf1 = (lambda f: (f.read(), f.close()))(open(r"D:\resources.arsc","rb"))[0]
    dic2 = ARSCParser(buf1).get_dict()
    print #dic2
    buf1 = (lambda f: (f.read(), f.close()))(open(r"D:\AndroidManifest.xml","rb"))[0]
    buf2 = AXMLPrinter(buf1).get_buff()
    print buf2
